...
const ServiceCardComponent = ({ service, layout = 'grid' }) => {
  ...
  const isImageValid = mainImage && failedUrl !== mainImage
  const isImageLoaded = loadedUrl === mainImage
        ...
        <div className="relative w-32 h-32 sm:w-40 sm:h-40 flex-shrink-0 rounded-2xl overflow-hidden bg-gray-50 border border-gray-100/50">
          {isImageValid ? (
            <img 
              ...
              onLoad={() => setLoadedUrl(mainImage)}
              onError={() => setFailedUrl(mainImage)}
              loading="lazy"
            />
          ) : (
          ...
          {isImageValid && !isImageLoaded && <div className="skeleton absolute inset-0" />}
